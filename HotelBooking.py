...
def create_database_and_tables(db_connection):
    ...
    cursor.execute("CREATE DATABASE IF NOT EXISTS hotelbk;")
    cursor.execute("USE hotelbk;")
...
class Admin:
    def __init__(self, db_connection):
        ...
        self.admin_credentials = {
            'admin1': 'admin123'  # Predefined username and password
        }
    ...
    def create_new_room(self, room_no, category_index, rate_per_day, hourly_rate=None):
    ...
    def search_room_by_bookid(self, booking_id):
        cursor = self.db_connection.cursor()
    ...
    def update_room_to_unoccupied(self, room_no):
        cursor = self.db_connection.cursor()
    ...
    def list_rooms_unoccupied(self):
        cursor = self.db_connection.cursor()
    ...
    def store_records_to_file(self, filename="hotel_records.txt"):
        cursor = self.db_connection.cursor()
        query = "SELECT * FROM Bookings"
        cursor.execute(query)
        bookings = cursor.fetchall()

        # Check if bookings were fetched
        if bookings:
            ...
            print("No bookings found to store in the file.")
...
class Customer:
    ...
    def register_customer(self, name, phone, email, user_id, password):
        ...
        if not self.validate_phone(phone):
            print("Invalid phone number. It should be numeric and have 10 digits, starting with 6, 7, 8, or 9.")
            return
        if not self.validate_email(email):
            print("Invalid email format. Only Gmail addresses are allowed.")
            return
...
def main():
    ...
    while True:
        ...
        if user_type == 1:
            ...
            if admin.login(user_id, password):
                while True:
                    print("\nAdmin Menu")
                    print("1. Create New Room")
                    print("2. Display the Rooms")
                    print("3. List Rooms : Occupied")
                    print("4. Search Room By BookId")
                    print("5. List Rooms : Unoccupied")
                    print("6. Update Room To Unoccupied")
                    print("7. Hotel Records")
                    print("8. Exit")
                    ...
                    if choice == 1:
                        ...
                        admin.create_new_room(room_no, category_index, rate_per_day, hourly_rate)
                    elif choice == 2:
                        admin.display_rooms()
                    elif choice == 3:
                        admin.list_rooms_occupied()
                    elif choice == 4:
                        booking_id = input("Enter booking ID: ")
                        admin.search_room_by_bookid(booking_id)
                    elif choice == 5:
                        admin.list_rooms_unoccupied()
                    elif choice == 6:
                        room_no = input("Enter room number to update to unoccupied: ")
                        admin.update_room_to_unoccupied(room_no)
                    elif choice == 7:
                        admin.store_records_to_file()
                    elif choice == 8:
                        break
                    else:
